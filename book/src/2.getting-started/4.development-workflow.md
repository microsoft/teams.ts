...
This guide covers best practices for developing Spark applications, including common development tasks, testing strategies, and deployment considerations.

## Development Cycle

A typical development workflow with Spark follows these steps:
...
The most common task in Spark development is adding handlers for different types of activities:
...
```typescript
import { AdaptiveCard, TextBlock, ActionSet, SubmitAction } from '@microsoft/spark.cards';
...
import { createTestClient } from '@microsoft/spark.dev/testing';
...
Spark applications can be hosted on various platforms:
...
2. **Unopinionated Design**: Use Spark as a flexible toolkit, not a rigid framework
3. **Tool Consolidation**: Leverage the integrated tools in the Spark ecosystem
...
2. **Build more complex applications** by combining different Spark packages
