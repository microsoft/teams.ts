...
| `ai` | A bot with AI capabilities using OpenAI |
...
```sh
teams new my-ttk-app --ttk basic
```
...
| `select <name>` | Select or create an environment. |
| `set <key> <value>` | Set an environment variable in the selected environment. |
| `del <key>` | Remove an environment variable in the selected environment. |
| `export` | Export environment variables to a .env file in the current working directory. |
