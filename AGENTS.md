...
- Release validation may run on Windows as well as Linux. Keep filesystem code and tests cross-platform: use Node and `path` APIs instead of hard-coded separators, specify UTF-8 for text APIs whose encoding is platform-dependent, and avoid locale-dependent or Unix-only filesystem assumptions.
