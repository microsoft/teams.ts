...
import { SocketActivityEnvelope, WS_CONNECT_PROTOCOL_VERSION } from './types';

describe('ws-connect envelope', () => {
  describe('readField', () => {
  ...
  describe('isInvokeEnvelope', () => {
    ...
    it('treats ackRequired as a one-way activity even when type=invoke', () => {
      // ackRequired means APX wants a delivery ack, not a client result.
      expect(isInvokeEnvelope({ type: 'invoke', ackRequired: true })).toBe(false);
    });
  ...
  describe('reply frames', () => {
    ...
    it('maps an InvokeResponse into a versioned invoke reply frame', () => {
      ...
      expect(frame.protocolVersion).toBe(WS_CONNECT_PROTOCOL_VERSION);
