import { ActivityLike, ConversationReference, DEFAULT_CLIENT_SETTINGS, isInvokeResponse } from '@microsoft/teams.api';
...
export async function $process<TPlugin extends IPlugin>(
  ...
  const apiClient = new ApiClient(serviceUrl, this.client.clone({ token: () => this.tokens.bot }), this.options.oauth?.clientSettings ?? DEFAULT_CLIENT_SETTINGS);
