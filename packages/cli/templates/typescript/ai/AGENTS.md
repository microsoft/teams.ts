...
This document provides guidance for AI coding agents working on this Microsoft Teams bot project built with the new Teams SDK and OpenAI integration.
...
2. **Correct Credential Names**: The Teams SDK uses `clientId`/`clientSecret`/`tenantId` - NOT the `MicrosoftApp*` names from older Bot Framework documentation.

3. **Credentials in App Constructor**: Environment variables alone aren't enough - you must pass credentials to the `App` constructor.

4. **SingleTenant Only**: Azure deprecated MultiTenant bots. Use SingleTenant and ensure you're using Teams in the same tenant.

5. **Globally Unique Bot Names**: Azure bot names must be unique across all of Azure. Add a timestamp suffix.
...
const app = new App({
  ...
  // IMPORTANT: Credentials are required for the bot to send responses back to Teams
  credentials: {
...
> **CRITICAL**: The `credentials` property is required for the bot to authenticate with Bot Framework and send responses. Without it, the bot will receive messages but fail to respond with `401 Unauthorized` errors.
...
- **Messaging Endpoint**: HTTPS URL where Teams will send messages (e.g., `https://your-domain.ngrok.io/api/messages`)

For local development, use [Microsoft Dev Tunnels](https://learn.microsoft.com/en-us/azure/developer/dev-tunnels/) to expose your local server. Dev tunnels are recommended over ngrok because they integrate with your Microsoft account and don't require a separate signup.
...
> **Note**: ngrok now requires an account and authtoken for all usage. If you prefer ngrok, sign up at https://dashboard.ngrok.com and run `ngrok config add-authtoken YOUR_TOKEN` before use.
...
**2. Wrong Credential Property Names**

The Teams SDK expects specific property names. Using `appId`/`appPassword` instead of `clientId`/`clientSecret` will cause silent auth failures with `Bearer null` in outgoing requests.

**Fix**: Use these exact names in your App config and .env:
- `clientId` (not `appId` or `MicrosoftAppId`)
- `clientSecret` (not `appPassword` or `MicrosoftAppPassword`)
- `tenantId` (not `appTenantId` or `MicrosoftAppTenantId`)

**3. Credentials Not Passed to App Constructor**

If credentials are only in `.env` but not passed to the `App` constructor, the bot won't authenticate.

**Fix**: Ensure your App initialization includes credentials:
```typescript
const app = new App({
  credentials: {
    clientId: process.env.CLIENT_ID,
...
ngrok now requires an account for all usage.

**Fix**: Either:
1. Sign up at https://dashboard.ngrok.com and run `ngrok config add-authtoken YOUR_TOKEN`
2. Or use Microsoft Dev Tunnels instead (recommended): `brew install --cask devtunnel`

### 401 Unauthorized When Bot Responds

Check the bot logs for `Authorization: 'Bearer null'`. This means credentials aren't being used.

**Fix**: Verify all three issues above (service principal, property names, App constructor).
